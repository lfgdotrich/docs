...
<Steps>
  <Step title="解析代币的 poolId">
    可用时使用 API 数据，然后 fallback 到 `hook.tokenToPoolId(tokenAddress)`。
  </Step>
  <Step title="解析或构建 PoolKey">
    优先使用 `factory.getPoolKey(tokenAddress)`，只有在已知常量时才手动构建。
  </Step>
  <Step title="通过 Hook 获取报价">
    V5 估算函数包含用户钱包地址，因为推荐路由会影响费用接收方。
  </Step>
  <Step title="通过 Swap Router 执行买入和卖出">
    交易执行使用该代币对应的 PoolKey。
  </Step>
  <Step title="通过 Hook 执行 borrow、borrow more 和 repay">
    借贷操作使用 `poolId`，抵押品授权给 Hook。
  </Step>
</Steps>
