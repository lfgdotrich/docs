---
title: Uniswap V4 Hook ABI
description: Hook runtime ABI for estimates, borrowing, pool state, and protocol callbacks.
---
...
<Note>
This is the deployed runtime ABI for connecting to the existing Uniswap V4 Hook contract. Constructor entries are not needed when creating `ethers.Contract`, `viem` contract calls, or Web3.py contract objects for an already-deployed contract.
</Note>
...
You can also import the matching JSON file from `/abis/UniswapV4Hook.abi.json` if your build setup supports JSON imports.
