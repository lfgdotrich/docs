...
1. resolves the token pool through the LFG.RICH Hook;
2. scans Hook `Buy` and `Sell` events;
3. builds internal price candles;
4. calculates indicators such as EMA, RSI, trend, pump, dump, and bleed conditions;
5. decides whether to `BUY`, `SELL`, or `HOLD`;
6. executes trades through the LFG.RICH Swap Router, unless `dry_run` is enabled;
7. records confirmed trades and lots in the local SQLite database.
...
1. latest valid candle close;
2. LFG Hook `getEffectivePrice(poolId)` fallback;
3. zero only if both are unavailable.
...
4. Run `python -m bot.smoke_test_lfg`.
5. Run the bot and confirm candles/signals look correct.
6. Test with `test_mode: true`, `test_action: "BUY"`, and a tiny `test_amount_bnb`.
7. Confirm the dashboard shows the created lot.
8. Only then disable `test_mode`.
